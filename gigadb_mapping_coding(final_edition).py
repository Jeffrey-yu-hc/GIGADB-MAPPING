...
# This is the main json file creation function.
def process_dataset(doi_number):
    url = f'https://gigadb.org/api/dataset?doi={doi_number}'
    try:
        response = requests.get(url)
        response.raise_for_status()  # Check the reponse status, and raise up warning if error
...
for doi_number in doi_list:
    try:
        url=f'https://gigadb.org/api/dataset?doi={doi_number}'
        response = requests.get(url)
        response.raise_for_status()  # 检查响应状态，如果是4xx或5xx则抛出错误
        # 调用 main 函数处理响应
        #main(response)
                # Example placeholders for extracted data from the 'main' function
        main(response)
